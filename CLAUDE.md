...
- Phone version: `milky-way-mobile.mp4`, 720×1280 portrait, 1.1 MB, with `milky-way-mobile-poster.jpg` (720×1280, 121 KB) as its first frame.
- `js/main.js` makes two separate decisions. **Which files:** 720px and below get the portrait pair, wider screens the landscape pair. The HTML's `poster` and `data-src` name the *mobile* files (phones are the most data-sensitive, and they are then right before JavaScript runs); `data-poster-wide` and `data-src-wide` hold the landscape ones. **Whether to play at all:** no video under reduced motion, data-saver, or `navigator.connection.effectiveType` of `slow-2g`/`2g`/`3g` — the poster shows instead and nothing downloads. The poster swap sits *outside* that second check, or a desktop with reduced motion would be left showing the portrait poster cropped into a wide box. It pauses once the whole sky area is scrolled out of view.
- Contrast over the portrait video was measured by compositing the poster through the hero overlay at 390×844: white headline median 11.7:1 (1st percentile 5.2:1), 78% soft sub-head median 7.9:1 — within a whisker of the landscape baseline at 1519×693 (13.2:1 / 8.7:1). No small-screen overlay override is needed: the radial is 75% of the *viewport width*, so on a narrow screen it covers nearly the full width and darkens the low galactic core instead of missing it.
